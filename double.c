...
IType* getDoubleType() {
    if (DOUBLE_TYPE_INSTANCE == NULL) {
        ...
        DOUBLE_TYPE_INSTANCE->funcs = malloc(sizeof(TypeFuncs));
        DOUBLE_TYPE_INSTANCE->size = sizeof(double);
        DOUBLE_TYPE_INSTANCE->typeId = 1;
        DOUBLE_TYPE_INSTANCE->funcs->add = doubleAdd;
        DOUBLE_TYPE_INSTANCE->funcs->mul = doubleMul;
        DOUBLE_TYPE_INSTANCE->funcs->print = doublePrint;
    }
...
void freeDouble() {
    if (DOUBLE_TYPE_INSTANCE) {
        free(DOUBLE_TYPE_INSTANCE->funcs);
